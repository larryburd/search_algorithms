...
int main(int argc, char* argv[]) {
    ...
    json_object_foreach(root, cityName, cityData) {
        ...
        json_t *weights = json_object_get(cityData, "weights");
        ...
        if (json_is_array(parents)) {
            ...
            json_array_foreach(parents, idx, parentNameObj) {
                ...
                // Get the cost for this edge
                float cost = 0.0;
                if (json_is_array(weights) && idx < json_array_size(weights)) {
                    json_t *weightObj = json_array_get(weights, idx);
                    cost = (float)json_number_value(weightObj);
                }

                // Add current city as child to the parent with cost
                if (parentIdx != -1 && childIdx != -1) {
                    addChild(graph.nodes[parentIdx], graph.nodes[childIdx], cost);
                }
            }
        }
    }
